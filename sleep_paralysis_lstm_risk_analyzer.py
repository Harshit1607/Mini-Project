...
Per-Subject Analysis Pipeline

This script replaces the old analyzer. It performs the correct workflow:
1.  Loads the trained CNN-LSTM model and the StandardScaler.
2.  Loops through each subject in the data directory.
3.  For each subject:
    a. Loads their full, continuous data.
    b. Extracts features and scales them.
    c. Creates a padded sequence for 1-to-1 prediction.
    d. Generates a full, continuous hypnogram using the model.
    e. Calculates the model's accuracy for this subject.
    f. Runs the SleepParalysisRiskAnalyzer on the clean hypnogram.
...
SEQUENCE_LENGTH = 20 # Must match the model!
NUM_FEATURES = 25    # Must match the model!

# --- File Paths ---
DATA_PATH = './sleep-accel-data/'
OUTPUT_DIR = './outputs'
MODEL_PATH = os.path.join(OUTPUT_DIR, 'best_cnn_lstm_model.keras')
...
def create_padded_sequences(X_scaled, sequence_length):
    """
    Creates overlapping sequences with zero-padding at the start
    to get a 1-to-1 prediction for every epoch.
    """
    num_epochs, num_features = X_scaled.shape
    
    # 1. Create padding
    padding = np.zeros((sequence_length - 1, num_features))
    
    # 2. Add padding to the beginning of the scaled data
    X_scaled_padded = np.vstack((padding, X_scaled))
    
    # 3. Create overlapping sequences
    X_seq = []
    for i in range(num_epochs): # Loop num_epochs times
        # Get window from i to i + sequence_length
        X_seq.append(X_scaled_padded[i : i + sequence_length])
        
    return np.array(X_seq)

# ========================================================================
# RISK ANALYZER CLASS (Copied from your original analyzer)
# ========================================================================
...
class SleepParalysisRiskAnalyzer:
    ...
    def __init__(self, epoch_duration=30):
        ...
        # --- NEW: Added a VERY_HIGH risk tier ---
    ...
    def _analyze_sleep_architecture(self, stages):
        # This is a simplified analysis and can be expanded
        architecture = {
            'sleep_cycles_detected': 0, 'notes': []
        }
        return architecture # Keeping it simple for now
...
def main():
    ...
    print("Running Per-Subject Analysis...")
    print("="*80)
    ...
    try:
        ...
        print("Please make sure you have run the *FIXED* classifier script first.")
    ...
    for i, subject_id in enumerate(subject_ids, 1):
        ...
        try:
            ...
            if len(features) <= SEQUENCE_LENGTH:
                print("  -> Skipping: Not enough data for a full sequence.")
                continue
            ...
            # Create Padded Sequences for 1-to-1 Prediction
            X_seq = create_padded_sequences(X_scaled, SEQUENCE_LENGTH)
    ...
    for r in all_reports:
        try:
            ...
            print(f"Warning: Could not parse report for subject {r['subject_id']}. Missing key: {e}")
    ...
    report_lines.append("Complete Sleep Paralysis Risk Assessment")
    report_lines.append("="*80)
    ...
    for level, pct in risk_dist.items():
        report_lines.append(f"  {level:<10}: {pct:5.1f}%")
